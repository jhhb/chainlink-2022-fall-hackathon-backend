  ...
  : describe("VRFD20 Unit Tests", async function () {
    ...
    describe('#fulfillRandomWords', async () => {
      ...
      it('allows the user to roll again on fulfillment', async () => {
        ...
        await expect (
          vrfConsumer.connect(account1).rollDice()
        ).to.be.revertedWith('You must wait for your current roll to complete before rolling again');
        ...
        ).to.emit(vrfConsumer, "DiceLanded").withArgs(initialRequestId, expectedRandomValue);

        const secondRequestId = 2;
        await expect ( vrfConsumer.connect(account1).rollDice() ).to.emit(vrfConsumer, "DiceRolled").withArgs(secondRequestId, account1.address);
      });
    });
    ...
    describe('#house', () => {
      ...
      describe("failure", async () => {
        it("reverts if no address has been recorded for the input address", async () => {
          const [account1] = await ethers.getSigners();
          await expect ( vrfConsumer.connect(account1).house(account1.address) ).to.be.revertedWith('The requested address must first call rollDice itself before a house is computed.');
        });

        it("reverts if the parameterized address is currently rolling", async () => {
          const [account1] = await ethers.getSigners();
          await vrfConsumer.connect(account1).rollDice();
          await expect ( vrfConsumer.connect(account1).house(account1.address)).to.be.revertedWith('The requested address is currently rolling. Please wait.');
        });
      });
      ...
      describe("success", () => {
        ...
        it("allows the sender to run the function for other addresses", async () => {
          ...
          const result = await vrfConsumer.connect(account2).house(accountWithResult.address);
          expect(result).to.eq("Lannister");
        });
      });
    });
